...
figma.ui.onmessage = (msg) => {
  if (msg.selectCheckboxOn === true) {
    async function alltoLayout() {
      ...
      if (msg.type === "fixedSize") {
        let fixedChild: any;
        const parentNodes = figma.currentPage.selection.filter(
          (node) => node.type === "FRAME"
        );

        parentNodes.forEach((parentFrames: FrameNode) => {
  ...
  if (msg.selectCheckboxOn === false) {
    async function alltoLayoutAll() {
      ...
      if (msg.type === "fixedSize") {
        let fixedChild: any;
        const parentNodes = figma.currentPage.findAll(
          (node) => node.type === "FRAME"
        );
